...
namespace drako::file_formats::obj
{
    namespace details
    {
        ...
        const std::string_view OBJ_IGNORED_KEYWORDS[] = {
        ...
        */
        ...
        [[nodiscard]] bool
        _is_ignored_keyword(const std::string_view _in_keyword) noexcept
        {
            return std::find(std::cbegin(OBJ_IGNORED_KEYWORDS),
                       std::cend(OBJ_IGNORED_KEYWORDS),
                       _in_keyword) != std::cend(OBJ_IGNORED_KEYWORDS);
        }
        ...
        _tokenize_line(const char* _in_begin, const char* _in_end, std::vector<_pt>& _out_tokens)
        {
            ...
            for (auto curr_line_index = 0; line_end != _in_end; ++curr_line_index)
            {
                ...
                if (std::string_view{ _out_tokens.back() } != OBJ_LINE_CONTINUATION)
                    break;
        ...
        _parse_v_line(const _pl& _in_line, const std::vector<_pt>& _in_tokens, std::vector<float>& _out_v) noexcept
        {
        ...
        [[nodiscard]] std::optional<parser_error_report>
        _parse_vn_line(const _pl& line, const std::vector<_pt>& tokens, std::vector<float>& vn) noexcept
        {
            using _pec = parser_error_code;
        ...
        [[nodiscard]] std::optional<parser_error_report>
        _parse_vt_line(const _pl& line, const std::vector<_pt>& tokens, std::vector<float>& vt) noexcept
        {
            using _pec = parser_error_code;
        ...
        [[nodiscard]] std::optional<parser_error_report>
        _parse_f_line(const _pl& line, const std::vector<_pt>& tokens, object& obj) noexcept
        {
            using _pec = parser_error_code;
        ...
        [[nodiscard]] std::optional<parser_error_report>
        _parse_o_line(Iter _in_first_token, Iter _in_last_token, object& _out_obj)
        {
        ...
        _parse_buffer_until_error(const char* _in_src_begin, const char* _in_src_end, const parser_config& _in_config)
        {
            ...
            for (const auto line_begin = _in_src_begin; line_begin != _in_src_end;)
            {
                ...
                } while (std::string_view{ tokens.back() } == OBJ_LINE_CONTINUATION);
                */
                _tokenize_line()
        ...
        _parse_buffer_skip_errors(const char* src_begin, const char* src_end, const parser_config& cfg)
        {
            using _ptk = _parser_token;
            using _pec = parser_error_code;
    ...
    {
        return details::_parse_buffer_until_error(source.data(), source.data() + std::size(source), config);
    }
    ...
    {
        return details::_parse_buffer_skip_errors(source.data(), source.data() + std::size(source), config);
    }
