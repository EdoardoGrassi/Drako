...
namespace drako::dod
{
    ...
    DRAKO_NODISCARD inline constexpr Iter
        find_first_min(Iter first, Iter last) noexcept
    {
        ...
        if (first == last) { return last; } // Range is empty

        // Linear search
        auto curr_min = first;
        while (++first != last)
        {
            if (*first < *curr_min) { curr_min = first; }
        }
        return curr_min;
    }
        ...
        find_first_max(Iter first, Iter last) noexcept
    {
        ...
        while (++first != last)
        {
            if (*first > *curr_max) { curr_max = first; }
        }
        return curr_max;
    }
    ...
    template <typename Iter>
    DRAKO_NODISCARD inline constexpr std::tuple<Iter, Iter>
