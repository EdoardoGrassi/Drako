...
namespace drako::gpx
{
        ...
        , _surface(surface)
    {
        ...
        auto [enumerate_pdevices_result, pdevices] = _instance.enumeratePhysicalDevices();
        if (enumerate_pdevices_result != vk::Result::eSuccess)
        {
        ...
        std::tie(result, _cmd_pool) = _ldevice.createCommandPool(pool_create_info);
        if (result != vk::Result::eSuccess)
        {
            std::exit(EXIT_FAILURE);
        }
        ...
        std::tie(result, _drawing_complete_sem) = _ldevice.createSemaphore({});
        if (result != vk::Result::eSuccess)
        {
            std::exit(EXIT_FAILURE);
        }

        std::tie(result, _present_complete_sem) = _ldevice.createSemaphore({});
        if (result != vk::Result::eSuccess)
        {
            std::exit(EXIT_FAILURE);
        }
